...
class CausalNet():
    ...
    def train(
        ...
        self._model = load_dataset_model(
            self.d,
            self.dataset_name
        )
        if warm_start:
        ...
        if self.dataset_name in ['celeA_complex']:
            imglabel = dataset.dataset.tensors[0]
            no_beard = imglabel[:, -2]
            bald = imglabel[:, -1]
            ...
            sampler = torch.utils.data.sampler.WeightedRandomSampler(sample_weights, len(imglabel), replacement=True)
    ...
    def compute_nll(self, dataset: Dataset) -> float:
        ...
        for batch in dataloader:
            ...
            output, attr_loss = self._model.pretrain(
                X_batch,
            )
            loss_details = attr_loss.detach().cpu().numpy()
            loss = attr_loss.sum(-1).mean()
            output_input_loss+= torch.nn.functional.l1_loss(output.squeeze(), X_batch, reduction='mean').item()
            total_loss += loss.item()
            loss_l1.append(loss_details)

        loss_attr_l1 = np.array([np.sqrt(x).mean(0) for x in loss_l1]).sum(0)/len(dataloader)

        l1_sum = loss_attr_l1.sum()
        l1_attrs_str = ', '.join([f'{x:.3f}' for x in loss_attr_l1])

        print(f"Testing: output_input_loss={int(output_input_loss/len(dataset))} l1_sum={l1_sum:.3f}, l1_attrs=[{l1_attrs_str}]")
...
def _train(
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % n_epochs_check == 0:
            ...
            loss_attr_l1 = np.array([np.sqrt(x).mean(0) for x in epoch_loss_details])
            loss_attr_l1 = loss_attr_l1.sum(0)/len(dataloader)
            ...
            log_str = f"Epoch {epoch}: loss={epoch_loss:.2f}, gamma={gamma:.2f}, l1_sum={l1_sum:.3f}, l1_attrs=[{l1_attrs_str}]"
            # Print to console
            print(log_str)
        ...
        if epoch % n_epochs_check_validation == 0 and val_dataloader is not None:
            ...
            for batch in val_dataloader:
                ...
                loss = attr_loss.sum(-1).mean()
                val_loss += loss.item()
                val_loss_l1.append(loss_details)
            
            loss_attr_l1 = np.array([np.sqrt(x).mean(0) for x in val_loss_l1]).sum(0)/len(dataloader)

            l1_sum = loss_attr_l1.sum()
            l1_attrs_str = ', '.join([f'{x:.3f}' for x in loss_attr_l1])

            log_str = f"Validation: {epoch}: l1_sum={l1_sum:.3f}, l1_attrs=[{l1_attrs_str}]"
            # Print to console
            print(log_str)
            ...
            if early_stopping and (not freeze_gamma_at_dag or gamma_cap is not None):
                ...
                if early_stopping_patience_counter >= early_stopping_patience:
                    ...
                    model = best_model
                    if logger_path is not None:
                        torch.save(best_model.state_dict(), os.path.join(os.path.dirname(logger_path),'best_model.pt'))
                    break
        #######################
        # End validation step #
        #######################
