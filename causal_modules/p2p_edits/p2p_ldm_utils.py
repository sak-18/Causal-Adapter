...
class LocalMask:
    ...
    def __call__(self, x_t, attention_store, step):
        if self.attn_mask_type == 'skip':
            return x_t
        k = 1
        # [len(layers),16,256,77]
        maps = attention_store["down_cross"][:2] + attention_store["up_cross"][3:6]
        maps = [item.reshape(self.alpha_layers.shape[0], -1, 1, 16, 16, MAX_NUM_WORDS) for item in maps]
        # cat over all timesteps in dim=1 [4, 4, 1, 16, 16, 77] -> [4, 4*5, 1, 16, 16, 77]
    ...
    def __init__(self, tokenizer: nn.Module, prompts: List[str], words: [List[List[str]]], \
                    ...
                    gpu_device= None, threshold: float = .3):
        """
        ...
        """
        # find the indices of words of attn_words
        ...
        #         words_ = [words_]
        #     for word in words_:
        #         # return the ind = origin_ind+1, [CLS] at start
        #         #ind = ptp_utils.get_word_inds(prompt, word, tokenizer)
        #         ind = torch.where(prompt == word)
        #         alpha_layers[i, :, :, :, :, ind] = 1
        # self.alpha_layers = alpha_layers.to(device)
        self.threshold = threshold
        self.attn_mask_type = attn_mask_type
        # find word indicied in opt.presudo_words_softmax
        ...
        if presudo_words_infonce[0] != '':
            ...
            for i, prompt in enumerate(prompts):
                for word in presudo_words_infonce:
                    #ind = ptp_utils.get_word_inds(prompt, word, tokenizer)
                    #ind = torch.where(prompt == word)
                    ind = torch.where(prompt == word)[0][0]
...
def run_and_display(ldm, prompts, controller, config, latent=None, run_baseline=True, \
    ...
    # try DDIM sampler
    images, x_t = ptp_utils.text2image_plms(ldm, controller, config, prompts, use_plms=False,\
