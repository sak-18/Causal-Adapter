...
import sys
sys.path.append("../../")
from ctf_datasets.transforms import SelectParentAttributesTransform
...
def get_dataloaders(data_class, attribute_size, config, transform=None, **kwargs):
    ...
    train_data_loader = torch.utils.data.DataLoader(train_set, batch_size=config["batch_size_train"], shuffle=True, num_workers=0)
    val_data_loader = torch.utils.data.DataLoader(val_set, batch_size=config["batch_size_val"], shuffle=False, num_workers=0)
    return train_data_loader, val_data_loader


def train_flow(flow, config, data_class, graph_structure, attribute_size, checkpoint_dir, **kwargs):
    transform = SelectParentAttributesTransform(flow.name[:-len('_flow')], attribute_size, graph_structure)
