...
def get_embedding_model(embedding, pretrained_vgg, classifier_config=None):
    if embedding == "vgg":
        ...
        if 'morphomnist' in classifier_config:
            ...
            model.load_state_dict(torch.load('../../methods/deepscm/morphomnist_uncond_image_vae-epoch=269.ckpt', map_location=torch.device('cuda'))["state_dict"])
            ...
            model.load_state_dict(torch.load('../../methods/deepscm/celeba_uncond_image_vae-epoch=44.ckpt', map_location=torch.device('cuda'))["state_dict"])
        else:
        ...
        cache_path = "./ViT-B-32.pt"
        model = clip.load(cache_path, device='cuda')[0]
...
def unnormalize_embedding_fn(unnormalize_fn, x, _,dtype=torch.uint8):
...
def vae_embedding_fn(embedding_model, x, cond):
    if hasattr(embedding_model, "module"):
        model = embedding_model.module
    else:
        model=embedding_model
    mu, logvar = model.encoder(x, cond)
    return np.transpose([mu.detach().cpu().numpy(), logvar.detach().cpu().numpy()], (1, 0, 2))
    #return np.stack([mu.detach().cpu().numpy(), logvar.detach().cpu().numpy()], axis=1)
