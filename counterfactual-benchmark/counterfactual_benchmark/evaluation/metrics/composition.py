...
def l1_distance(images, cond, steps, embedding, embedding_fn):
    ...
    for step in steps:
        if embedding == "lpips":
            ...
            #distances[step] = np.mean(np.abs(embedding_fn(images[step], cond).astype(int) - embedding_fn(images[0], cond).astype(int)), axis=(1,2,3) if embedding is None else 1)
