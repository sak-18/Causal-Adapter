...
class BalancedAttributeSampler(Sampler):
    def __init__(self, imglabel, attr_indices, batch_size, samples_per_epoch=None, max_per_sample_repeat=10):
        """
        Args:
            imglabel (Tensor): shape [N, D]
            attr_indices (tuple): indices of No_Beard and Bald in imglabel
            batch_size (int): batch size
            samples_per_epoch (int): optional, total samples per epoch
            max_per_sample_repeat (int): max number of times a sample can be used per epoch
        """
        self.batch_size = batch_size
        self.max_per_sample_repeat = max_per_sample_repeat

        # Build condition → index map
        self.cond_to_indices = defaultdict(list)
        for idx, label in enumerate(imglabel):
            no_beard = int(label[attr_indices[0]])
            bald = int(label[attr_indices[1]])
            key = (no_beard, bald)
            self.cond_to_indices[key].append(idx)

        self.conditions = list(self.cond_to_indices.keys())
        self.condition_counts = {cond: len(indices) for cond, indices in self.cond_to_indices.items()}
        self.condition_weights = self._compute_inverse_weights()

        self.samples_per_epoch = samples_per_epoch or sum(self.condition_counts.values())

    def _compute_inverse_weights(self):
        total = sum(self.condition_counts.values())
        weights = {
            cond: total / (len(self.conditions) * count)
            for cond, count in self.condition_counts.items()
        }
        return weights

    def __iter__(self):
        sample_indices = []
        sample_use_counter = Counter()

        while len(sample_indices) < self.samples_per_epoch:
            batch = []

            while len(batch) < self.batch_size:
                cond = random.choices(
                    population=self.conditions,
                    weights=[self.condition_weights[c] for c in self.conditions],
                    k=1
                )[0]

                candidate_pool = self.cond_to_indices[cond]

                # Filter out samples already used max times
                valid_candidates = [idx for idx in candidate_pool if sample_use_counter[idx] < self.max_per_sample_repeat]

                if not valid_candidates:
                    continue  # skip this condition if exhausted

                selected_idx = random.choice(valid_candidates)
                sample_use_counter[selected_idx] += 1
