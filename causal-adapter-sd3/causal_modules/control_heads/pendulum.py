...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def __init__(
        ...
        #self.nonlinearities_noises = nn.ModuleDict()
        for i in range(self.in_dim):
            # self.nonlinearities_noises[str(i)] = MLP(latent_dim=4
            #                                     ,num_var=4,use_bias=False)
    ...
    def pretrain(self, label):
        ...
        for i in range(self.in_dim):
            ...
            if len(result_v_indices) > 0 and i in result_v_indices:
                z_post[:,i] = z_i_out
                #z_post[:, i] = z_i_out+self.nonlinearities_noises[str(i)](mu[:,i].unsqueeze(1)).squeeze(-1)
            else:
                z_post[:, i] = mu[:, i]
