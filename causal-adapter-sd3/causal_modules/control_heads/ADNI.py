...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def inference(self,label,sample=False,intervention_indx=None,intervention_values=None,disentangle=False):
        ...
        if intervention_indx !=None and intervention_values!=None:
            # do intervention
            # [attr1,attr2,age,sex,b_v,ven_v,slice 0-9]
            if mu.shape[1]==16:
                mu = transform_attributes(mu)

            reason_v_indices, result_v_indices = self.get_result_variable_indices(self.mask)
            if intervention_indx == 0:
                intervention_values = bin_array(intervention_values,reverse=True) 
            elif intervention_indx == 5:
                intervention_values = ordinal_array(intervention_values,reverse=True)
            ...
            if intervention_indx !=None:
                ...
                if intervention_indx in [0,1,2,3,6]:
                    ...
                    z =  transform_attributes(label.to(device))
                    z[:, intervention_indx] = intervention_values
                
            output = transform_attributes(z,reverse=True).unsqueeze(2)  # [bs, num_var, 1]
        else:
            # do reconstruction
    ...
    def pretrain(self, label):
        ...
        device = self.device
        mu = label.to(device)
        input_feature_dim  = mu.shape[1]
        if input_feature_dim==16:
            mu = transform_attributes(mu)
...
class ControlNetConditioningEmbedding_discovery(nn.Module):
    ...
    def forward(self, label):
        ...
        if input_feature_dim==16:
            output = transform_attributes(z_post,reverse=True).unsqueeze(2)  # [bs, num_var, 1]
        else:
            output = z_post.unsqueeze(2)
