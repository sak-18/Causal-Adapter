...
class AutoEncoderLayers_causalnet(nn.Module):
    def __init__(
        self,
        in_dim,
        hidden_dims,
    ...
    def reconstruction_loss(self, x, mask_interventions_oh=None):
        ...
        mean_attr_l1 = torch.nn.functional.l1_loss(x_mean,x,reduction='none').mean(0).detach().numpy()
        #print('l2 loss is {}, sum {}'.format(mean_attr_l2,sum(mean_attr_l2)))
        return nll,mean_attr_l1
    
    def reconstruction_loss_l2(self, x, mask_interventions_oh=None):
        x_mean, x_var = self(x)

        if mask_interventions_oh is None:
            mask_interventions_oh = torch.ones_like(x_mean)

        nll = torch.nn.functional.mse_loss(x_mean,x,reduction='none')
    ...
    def loss(
        ...
        nll,mean_attr_l1 = self.reconstruction_loss_l2(x, mask_interventions_oh=mask_interventions_oh)
        #nll,mean_attr_l1 = self.reconstruction_loss_l2(x, mask_interventions_oh=mask_interventions_oh)
        alpha,beta = 0,0
        l1_reg = alpha * self.l1_reg_dispatcher()  # * n_obs_norm
        l2_reg = beta * self.l2_reg_all_weights()  # * n_obs_norm
