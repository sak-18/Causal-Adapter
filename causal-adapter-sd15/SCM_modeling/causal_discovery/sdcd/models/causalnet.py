...
AutoEncoderLayers = modules.AutoEncoderLayers
AutoEncoderLayers_causalnet = modules.AutoEncoderLayers_causalnet
...
print(_DEFAULT_STAGE1_KWARGS['batch_size'])
...
class CausalNet(BaseModel):
    ...
    def train(
        ...
        self._model = AutoEncoderLayers_causalnet(
            self.d,
            [self._model_kwargs["dim_hidden"]] * self._model_kwargs["num_layers"],
            nn.LeakyReLU(),
            model_variance_flavor=self.model_variance_flavor,
    ...
    def compute_nll(self, dataset: Dataset) -> float:
        ...
        for batch in dataloader:
            ...
            loss,loss_details = self._model.reconstruction_loss(
                    X_batch,
        ...
        loss_attr_l1 = np.array(loss_l1).sum(0)/len(dataloader)

        l1_sum = loss_attr_l1.sum()
        l1_attrs_str = ', '.join([f'{x:.3f}' for x in loss_attr_l1])

        print(f"Testing: l1_sum={l1_sum:.3f}, l1_attrs=[{l1_attrs_str}]")
...
def _train(
    ...
    for epoch in range(n_epochs):
        ...
        if epoch % n_epochs_check == 0:
            ...
            loss_attr_l1 = np.array([loss_detail['attr_l1'] for loss_detail in epoch_loss_details])
            ...
            if B_true is not None:
                ...
                print(f"Epoch {epoch}: loss={epoch_loss:.2f}, gamma={gamma:.2f}, l1_sum={l1_sum:.3f}, l1_attrs=[{l1_attrs_str}]")
        ...
        if epoch % n_epochs_check_validation == 0 and val_dataloader is not None:
            ...
            for batch in val_dataloader:
                ...
                loss,loss_details = model.reconstruction_loss(
                    X_batch,
                    mask_interventions_oh=mask_interventions_oh,
                )
                val_loss += loss.item()
            ...
            loss_attr_l1 = np.array(val_loss_l1).sum(0)/len(dataloader)

            l1_sum = loss_attr_l1.sum()
            l1_attrs_str = ', '.join([f'{x:.3f}' for x in loss_attr_l1])

            print(f"Validation: {epoch}: l1_sum={l1_sum:.3f}, l1_attrs=[{l1_attrs_str}]")
            if log_wandb:
                wandb.log(
                    {
                        "epoch": epoch + start_wandb_epoch,
