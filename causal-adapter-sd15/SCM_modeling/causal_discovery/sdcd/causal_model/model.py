...
class CausalModel:
    ...
    def sample_from_model(self, n_samples, intervention_name=None):
        ...
        for node in nx.topological_sort(self.graph):
            ...
            # Margin distribution don't use the parent value for sampling, perform like the intervention
            if node in interventions:
                distribution = interventions[node]
            else:
                distribution = self.causal_mechanisms[node]
            # if the nodes have patents, although it sample_shape=[], it still get the value from patent from N(torch.size(n_sample))
            sample_shape = (
                [n_samples]
    ...
    def generate_dataframe_from_all_distributions(
        ...
        for intervention_name in self.interventions.keys():
            ...
            data.append(samples)
        # (1200,21) = 200(n_samples_control) + 50(n_samples_per_intervention) * 20(n_interventions), final column is the label of intervention idx
