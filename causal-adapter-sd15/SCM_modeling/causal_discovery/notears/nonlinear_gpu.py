...
class NotearsMLP(nn.Module):
    def __init__(self, dims, bias=True):
        ...
        d = dims[0]
        self.dims = dims
        # fc1: variable splitting for l1 [4,4*10]
        self.embed_linear = nn.Linear(768,1)
    ...
    def forward(self, x,emb):  # [n, d] -> [n, d]
...
def dual_ascent_step(model, X,embeddings, lambda1, lambda2, rho, alpha, h, rho_max):
    ...
    while rho < rho_max:
        def closure():
            optimizer.zero_grad()
            X_hat = model(X_torch,embeddings.type(X_torch.dtype))
...
def notears_nonlinear(model: nn.Module,
    ...
    # X [n_samples, attributes] 
    
    rho, alpha, h = 1.0, 0.0, np.inf
    for _ in range(max_iter):
        rho, alpha, h = dual_ascent_step(model, X,embeddings, lambda1, lambda2,
                                         rho, alpha, h, rho_max)
