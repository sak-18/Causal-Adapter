...
def log_validation(text_encoder, tokenizer, unet, vae, args, accelerator, weight_dtype, epoch):
    ...
    images = pipeline(args.validation_prompt, num_inference_steps=100, generator=generator,guidance_scale=7.5,num_images_per_prompt=args.num_validation_images,height=args.resolution,width=args.resolution).images
    #     with autocast_ctx:
...
class TextualInversionDataset(Dataset):
    def __init__(
        ...
        assert dataset in ['pendulum','celeA','skin_cancer','chexpert']
        ...
        if dataset == 'pendulum':
            ...
            csv_path = '/home/jovyan/fcvm-data-volume/kzzr229/workspace/causal_datasets/celeA/bald_subset.csv'
            df = pd.read_csv(csv_path)
            img_names =np.asarray(df.Image_Name)
            ...
            self.image_paths = [os.path.join(self.data_root, file_path) for file_path in img_names]
            label_list = np.asarray(df[select_columns])
            mapping_age = {'female': 0, 'male': 1}
            mapping_dx = {'mel': 0, 'nv': 1}
            label_list[:, 0] = np.vectorize(mapping_age.get)(label_list[:, 0])
            label_list[:, 2] = np.vectorize(mapping_dx.get)(label_list[:, 2])
