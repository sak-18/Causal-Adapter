...
class TextualInversionLoaderMixin:
    ...
    def load_mcpl_inversion(
        ...
        **kwargs,
        ...
        tokens = []
        embeddings = []
        for key,embed in state_dicts[0].items():
            tokens.append(key)
        ...
        presudo_token_ids = tokenizer.encode(tokens, add_special_tokens=False)
        ...
        # 7.4 Load token and embedding
        for token_id, embedding in zip(presudo_token_ids, embeddings):
            # add tokens and get ids
            # tokenizer.add_tokens(token)
            # token_id = tokenizer.convert_tokens_to_ids(token)
            input_embeddings.data[token_id] = embedding
