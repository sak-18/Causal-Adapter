...
#                     outs = sampler.p_sample_ddim(latents, cond, ts, index=index,
#                                         unconditional_guidance_scale=guidance_scale,
#                                         unconditional_conditioning=uc,)
...
#     """
#     # self is the cross attention layer
...
class AttendExciteCrossAttnProcessor:
    ...
    def __call__(self, attn, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        self.attnstore(attention_probs.clone(), is_cross, self.place_in_unet)
...
def get_word_inds(text: str, word_place: int, tokenizer):
    ...
    To carefull check whether or not has word been spliited by the tokenizers, e.g. tochat ->['to','chat']->return the two indices
    Args:
        text (str): Input text.
    ...
    out = []
    if len(word_place) > 0:
        # remove [CLS] [SEP] at the start and end token
