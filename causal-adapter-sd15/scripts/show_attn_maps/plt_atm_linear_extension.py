...
def load_mcpl_embeddings(base_model_path,tokenizer,embedding_path):
...
if __name__ == "__main__":
    ...
    text_encoder = load_mcpl_embeddings(model_id,tokenizer,mcpl_embedding_path)
    #cover the new text_encode into the pipeline, otherwise load the original weight
    pipe = StableDiffusionPipeline.from_pretrained(
        model_id,text_encoder=text_encoder ,torch_dtype=torch.float32
    )
